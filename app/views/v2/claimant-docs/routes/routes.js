...
const versionPath = "/v2/claimant-docs";
...
router.all(`${versionPath}*`, (req, res, next) => {
  ...
  res.locals.unreadCount =
    activeDocs.length - activeDocs.filter((x) => x.read).length;

  res.locals.lastUpdatedTime = new Date().toLocaleString("en-GB", {
    hour: "numeric",
    minute: "numeric",
    hour12: true,
  });
...
router.get(`${versionPath}/view-document/:id`, (req, res) => {
  ...
  return res.render(`v2/claimant-docs/view-document`);
});
...
router.get(`${versionPath}/remove-document/:id`, (req, res) => {
  ...
  return res.render(`v2/claimant-docs/remove-document`);
});
...
router.get(`${versionPath}/update-document/:id`, (req, res) => {
  ...
  return res.render(`v2/claimant-docs/update-document`);
});
...
router.post(`${versionPath}/update-document/:id`, (req, res) => {
  ...
  if (months[doc.dateMonth - 1]) {
    doc.dateOfDocument = `${doc.dateDay} ${months[doc.dateMonth - 1]} ${
      doc.dateYear
    }`;
  }
  ...
  if (!req.query.ncat && allowedUrls.includes(url_view_document)) {
    return res.redirect(url_view_document);
  } else {
...
router.get(`${versionPath}/clear-filters`, (req, res) => {
  ...
  return res.render(`v2/claimant-docs/document-list`);
});
...
router.get(`${versionPath}/pin/:id`, (req, res) => {
  ...
  const returnTo =
    req.query.return === "view"
      ? `${versionPath}/view-document/${
          doc.id
        }?msg=pinned&name=${encodeURIComponent(doc.name)}`
      : `${versionPath}/document-list?msg=pinned&name=${encodeURIComponent(
          doc.name
        )}`;
...
router.get(`${versionPath}/unpin/:id`, (req, res) => {
  ...
  const returnTo =
    req.query.return === "view"
      ? `${versionPath}/view-document/${
          doc.id
        }?msg=unpinned&name=${encodeURIComponent(doc.name)}`
      : `${versionPath}/document-list?msg=unpinned&name=${encodeURIComponent(
          doc.name
        )}`;
...
router.get("/v2/claimant-docs/view-document/:id", (req, res) => {
  const documents = req.session.data.documents || [];

  // find the clicked document
  const doc = documents.find((d) => d.id === req.params.id);
  ...
  // save back to session (important)
  req.session.data.documents = documents;

  res.render("v2/claimant-docs/view-document", {
    doc,
  });
});
